...
class LamaHttpClient {
  ...
  // Chat operations
  async sendMessage(params: { conversationId: string; content: string; senderId?: string }) {
  ...
  chatMemoryHandler = {
    enableMemories: async (topicId: string, autoExtract = true, keywords: string[] = []) => {
      return this.post('/api/chatMemory/enableMemories', { topicId, autoExtract, keywords });
    },
    disableMemories: async (topicId: string) => {
      return this.post('/api/chatMemory/disableMemories', { topicId });
    },
    toggleMemories: async (topicId: string) => {
      return this.post('/api/chatMemory/toggleMemories', { topicId });
    },
    extractSubjects: async (params: { topicId: string; limit?: number; includeContext?: boolean }) => {
      return this.post('/api/chatMemory/extractSubjects', params);
    },
    findRelatedMemories: async (topicId: string, keywords: string[], limit = 10) => {
      return this.post('/api/chatMemory/findRelatedMemories', { topicId, keywords, limit });
    },
    getMemoryStatus: (topicId: string) => {
...
class AIAssistantHttpProxy {
  constructor(private client: LamaHttpClient) {}
  ...
  async getOrCreateAITopic(modelId: string) {
    // TODO: Implement via HTTP API
    throw new Error('AI topic creation not yet supported via HTTP API');
  }

  async generateResponse(params: { message: string; modelId: string; topicId?: string }) {
    // TODO: Implement via HTTP API
 ...
 * Parse CLI arguments
 */
function parseArgs(): { url: string; help: boolean } {
...
async function main() {
  ...
  try {
    ...
    console.error('[LAMA MCP] Error:', (error as Error).message);
  ...
  // Create MCP server
  const mcpServer = new LamaMCPServer(httpClient as any, aiAssistant as any);
